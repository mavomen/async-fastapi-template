...
class TestOAuthLoginEndpoint:
    def test_login_returns_authorize_url(self, client, mocker):
        mocker.patch(
            "app.identity.auth.oauth2._get_client_credentials",
            return_value=("google-id", "google-secret"),
        )
    ...
    def test_login_with_github(self, client, mocker):
        mocker.patch(
            "app.identity.auth.oauth2._get_client_credentials",
            return_value=("github-id", "github-secret"),
        )
...
class TestOAuthCallbackEndpoint:
    def test_callback_with_invalid_state(self, client, mocker):
        mocker.patch("app.identity.api.endpoints.auth.consume_oauth_state", return_value=None)
        mocker.patch(
            "app.identity.auth.oauth2._get_client_credentials", return_value=("id", "secret")
        )
    ...
    def test_callback_with_exchange_failure(self, client, mocker):
        mocker.patch(
            "app.identity.api.endpoints.auth.consume_oauth_state",
        ...
        mocker.patch(
            "app.identity.api.endpoints.auth.exchange_code", side_effect=Exception("bad code")
        )
    ...
    def test_callback_with_userinfo_failure(self, client, mocker):
        ...
        mocker.patch(
            "app.identity.api.endpoints.auth.exchange_code",
            return_value={"access_token": "at", "refresh_token": "rt"},
        )
        mocker.patch(
            "app.identity.api.endpoints.auth.get_user_info", side_effect=Exception("API error")
        )
    ...
    def test_callback_creates_new_user(self, client, mocker):
        ...
        mocker.patch(
            "app.identity.api.endpoints.auth.get_user_info",
            return_value={"email": "new@example.com", "name": "New", "id": "999"},
        )
        mocker.patch("app.identity.api.endpoints.auth.crud_user.get_by_oauth", return_value=None)
        mocker.patch("app.identity.api.endpoints.auth.crud_user.get_by_email", return_value=None)
        mocker.patch("app.identity.api.endpoints.auth.crud_user.create_oauth_user")
        mocker.patch(
    ...
    def test_callback_links_existing_user(self, client, mocker):
        ...
        mocker.patch(
            "app.identity.api.endpoints.auth.exchange_code", return_value={"access_token": "at"}
        )
        mocker.patch(
            "app.identity.api.endpoints.auth.get_user_info",
        ...
        mocker.patch("app.identity.api.endpoints.auth.crud_user.get_by_oauth", return_value=None)
        mocker.patch(
            "app.identity.api.endpoints.auth.crud_user.get_by_email", return_value=mock_user
        )
        mocker.patch(
            "app.identity.api.endpoints.auth.crud_user.link_oauth_account", return_value=mock_user
        )
        mocker.patch(
            "app.identity.api.endpoints.auth._issue_tokens",
            return_value={"access_token": "at", "refresh_token": "rt", "token_type": "bearer"},
    ...
    def test_callback_returns_existing_user_by_oauth(self, client, mocker):
        ...
        mocker.patch(
            "app.identity.api.endpoints.auth.crud_user.get_by_oauth", return_value=mock_user
        )
        mocker.patch(
            "app.identity.api.endpoints.auth._issue_tokens",
            return_value={"access_token": "at", "refresh_token": "rt", "token_type": "bearer"},
    ...
    def test_callback_no_email(self, client, mocker):
        ...
        mocker.patch(
            "app.identity.api.endpoints.auth.get_user_info",
            return_value={"name": "No Email", "id": "123"},
        )
