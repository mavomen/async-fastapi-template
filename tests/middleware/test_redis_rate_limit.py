...
class TestHelperFunctions:
    ...
    def test_get_tier_config_admin(self):
        with patch("app.middleware.redis_rate_limit.settings", _DEV_SETTINGS):
            tier, limit, window = _get_tier_config("/admin/users")
        assert tier == "admin"
...
class TestRedisRateLimitMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_returns_429_when_blocked(self):
        request = MagicMock()
