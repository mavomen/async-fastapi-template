...
async def auth_headers(db_session: AsyncSession) -> dict:
    user = await crud_user.create(
        db_session,
        obj_in=UserCreate(
            email="webauthn@test.com", username="webauthn", password="Password1!"
        ),
    )
...
async def test_webauthn_register_begin(async_client: AsyncClient, auth_headers: dict):
    ...
    with patch("app.api.endpoints.auth.begin_registration") as mock_begin:
        ...
        resp = await async_client.post(
            "/api/v1/auth/webauthn/register/begin", headers=auth_headers
        )
...
async def test_webauthn_login_begin(async_client: AsyncClient):
    ...
    with patch("app.api.endpoints.auth.begin_authentication") as mock_begin_opt, patch(
        "app.core.database.sessionmanager.session"
    ) as mock_sess, patch(
        "app.api.endpoints.auth.crud_user.get_by_email"
    ) as mock_get_user:
